...
//extern static packet_t circ_buf[CIRC_BUF_SIZE];
...
uint32_t USB_Data_Peek(uint8_t *data)
{
 ...
 // data = circ_buf[head].pbuf;
  //data = (uint8_t *)circ_buf[head].pbuf;
  //data = (uint8_t *)&circ_buf[head].pbuf;
 ...
 // printf("\n\r Data peek\n\r");
  ...
  for (uint8_t x=0;x<circ_buf[head].len;x++)
  {
    data[x] = circ_buf[head].pbuf[x];
    //data[x]=0xaa;
    //printf("0x%x,",(uint8_t*) data[x]);
   // printf("0x%x,",data[x]);

  }
 // printf("data test two %x",data);
  printf("\n\r");
  //printf("Data Peek  0x%x,0x%x\n\r",*circ_buf[head].pbuf,data);
  //printf("usb peek return %d at head %d tail %d\n\r",circ_buf[head].pbuf[0] ,head,tail);
