...
static int _np_cmd_nand_read(np_prog_t *prog)
{
    ...
    DEBUG_PRINT("Read at 0x%" PRIx64 " 0x%" PRIx64 " bytes \r\n", addr,
        len);
...
static void np_packet_handler(np_prog_t *prog)
{
    ...
    do
    {
        ...
        prog->rx_buf_len = np_comm_cb->peek(&buf);
        prog->rx_buf = buf;
        if (!prog->rx_buf_len)
            break;
      //  printf("\n\rpacket peek L=%d  --  \n\r",prog->rx_buf_len);
      ...
      //  printf("0x%x\n\r",prog->rx_buf);
      //  printf("0x%x\n\r",*prog->rx_buf);
       /* for (uint8_t x=0;x<prog->rx_buf_len;x++)
        {
       //     printf("0x%x,",prog->rx_buf[x]);
            //printf("0x%x,",buf[x]);
        }
       // printf("\n\rSecond buffer\n\r");
          for (uint8_t x=0;x<prog->rx_buf_len;x++)
        {
            //printf("0x%x,",(uint8_t *)prog->rx_buf[x]);
        //    printf("0x%x,",buf[x]);
        }
       // printf("\n\r");*/
...
void np_init()
{
    prog_holder.active_image = 0xff;
}

void np_handler()
{
    np_packet_handler(&prog_holder);
    np_nand_handler(&prog_holder);
}
